 ...
 <form class="form-inline" method="get">
   <div class="form-group">
    <select name="filter" class="form-control" onchange="form.submit()">
     ...
     <option value="1" <?php if($filter == 'Tetap'){ echo 'selected'; } ?>>Cuero</option>
     <option value="2" <?php if($filter == 'Kontrak'){ echo 'selected'; } ?>>Alcántara</option>
     <option value="3" <?php if($filter == 'Kontrak'){ echo 'selected'; } ?>>Vinilo</option>
     <option value="4" <?php if($filter == 'Kontrak'){ echo 'selected'; } ?>>Microfibra</option>
   </select>
 </div>
</form>
<br />
<div class="panel-body">
 <table class="table table-bordered table-striped">
  ...
  <th class="text-center" style="width: 10%;">Tela a utilizar</th>
 ...
 if(mysqli_num_rows($sql) == 0){
   ...
   <td class="text-center">'.$row['ano'].'</td>
    ...
    if($row['tela'] == '1'){
      echo '<span class="label label-success">Cuero</span>';
   }
   else if($row['tela'] == '2'){
     echo '<span class="label label-info">Alcántara</span>';
   }
   else if($row['tela'] == '3'){
    echo '<span class="label label-warning">Vinilo</span>';
   }
   else if($row['tela'] == '4'){
    echo '<span class="label label-warning">Microfibra</span>';
   }
   echo '
   </td>
   ...
   <a href="editar_cliente.php?nik='.$row['nit'].'" title="Editar datos" class="btn btn-xs btn-warning"><span class="glyphicon glyphicon-edit" aria-hidden="true"></span></a>
